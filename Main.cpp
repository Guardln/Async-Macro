...
// Check if file at path exist - used for configs folder // C:\configs 
BOOL dex(LPCTSTR szPath)
{
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        ImGui::NewFrame();
        {
            ...
            if (!init) {
                ...
                std::thread(macro::macroThread).detach();
                std::thread(macro::keysThread).detach();
                init = true;
            }
            else 
            {
